...
- **房主人走了、没人能管账怎么办**：非房主的看板上会有一行"Host gone? Take over"。任何知道这局 PIN 的人都能点这个，输入现在的 PIN 证明确实知情，同时**必须**设一个新 PIN——接管成功后，旧房主那台设备/账号会立刻失去权限（`host_token` 和 `created_by` 都会被换掉），新 PIN 只有接管的人知道。
